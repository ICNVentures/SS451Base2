four51.app.controller('Four51Ctrl', ['$scope', '$route', '$location', '$451', 'User', 'Order', 'Security', 'OrderConfig', 'Category', 'AppConst','XLATService', 'GoogleAnalytics', 'SmartSource', 'SpendingAccount',
function ($scope, $route, $location, $451, User, Order, Security, OrderConfig, Category, AppConst, XLATService, GoogleAnalytics, SmartSource, SpendingAccount) {
	...
	function init() {
		if (Security.isAuthenticated()) {
			User.get(function (user) {
                ...
                if (fld == "SSBLandingOptionBan" || fld == "SSBLandingOptionCat")
                {
                    $scope.showHome = true;
                }
			});
