four51.app.factory('ProductDisplayService', ['$sce', '$451', 'Variant', 'Product', 'SmartSource',
function($sce, $451, Variant, Product, SmartSource){
	...
	function productViewScope(scope){
		...
		scope.setAddToOrderErrors = function(){
			...
			if(!scope.LineItem.Variant && scope.LineItem.Product.IsVBOSS){
				newErrorList.push("The customization selected below is invalid or not available for this product.");
			}
        ...
        scope.shoppingList = '';
        scope.usesFaves = false;
        
        if (typeof scope.tree !== "undefined" && scope.tree !== null && scope.tree.length > 0)
        {
			for(var i = 0; i < scope.tree.length; i++)
			{
                if (scope.tree[i].Name === 'Favorites')
                {
                    scope.usesFaves = true;
                    break;
                }
			}
        }
        ...
        if (scope.usesFaves)
        {
            ...
            SmartSource.shoppinglist(scope.user.InteropID, function(list) 
            {
                if (list !== null && list !== '' && list.length > 0)
                {
                    scope.shoppingList = list.split('|');
                        
                    if (scope.shoppingList.indexOf(scope.LineItem.Product.ExternalID) >= 0)
                    {
                        scope.faveHtml = '-';
                    }
                    else
            ...
            scope.favoriteAddDel = function(prod) 
            {
                SmartSource.shoppinglist(scope.user.InteropID, function(list) 
                {
                    if (list !== null && list !== '' && list.length > 0)
                    ...
                    if (scope.shoppingList.indexOf(prod) >= 0)
                    {
                        SmartSource.shoppinglistDel(scope.user.InteropID, prod, function(list)
                        {
                            scope.shoppingList = list;
                            scope.faveHtml = '+';
                        });
                    ...
                    {
                        SmartSource.shoppinglistAdd(scope.user.InteropID, prod, function(list)
                        {
                            scope.shoppingList = list;
                            scope.faveHtml = '-';
                        });
