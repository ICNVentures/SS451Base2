...
function LargeShipAddressSearchCtrl($scope, AddressList, LargeAddressList, Address) {
    ...
    $scope.$watch('ShipAddress', function(newValue) 
    {
        ...
        else 
        {
            ...
            if ($scope.orderShipAddress)
            {
                fn =  $scope.orderShipAddress.FirstName;
                ln =  $scope.orderShipAddress.LastName;
            }
            else
            {
                fn = $scope.currentOrder.LineItems[0].ShipFirstName;
                ln = $scope.currentOrder.LineItems[0].ShipLastName;
            }
            
            //if ($scope.currentOrder && $scope.currentOrder.Shipper)
            //{
			    //var sh = $scope.currentOrder.Shipper;
		        //var si = $scope.currentOrder.ShipperID;
                //console.log('else[' + sh.ID + '][' + si + ']');
            //}
            $scope.orderShipAddress = newValue;
            
            if (fn !== '')
            {
                $scope.orderShipAddress.FirstName = fn;
            }
            
            if (ln !== '')
            {
                $scope.orderShipAddress.LastName = ln;
            }
            ...
            if (fn !== '')
            {
                $scope.currentOrder.ShipFirstName = fn;
            }
            
            if (ln !== '')
            {
                $scope.currentOrder.ShipLastName = ln;
            }
            ...
            if ($scope.currentOrder) 
            {
                ...
                //Never wipe out ship first and last names
            ...
            if (newValue) 
            {
                //Never change ship first and last names, ever.
